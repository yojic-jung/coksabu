...
public class AdminController {
	...
	@RequestMapping("dbMirgrationForCate")
	public String dbMirgrationForCate(HttpSession session) {
		GenericXmlApplicationContext ctx = new GenericXmlApplicationContext("classpath:/applicationContext.xml");
		AdminAdditionService adminAddService = ctx.getBean(AdminAdditionService.class);
		adminAddService.dbMirgrationForCate();
		ctx.close();
	...
	public String admininspect(Certify certify, HttpServletRequest request, Model model)
	...
	public String certifyFail(@RequestParam("denyReason") String denyReason, HttpSession session,
	...
	public String changeOrderStatus(HttpSession session, OrderStatusRecord statusRecord, HttpServletRequest request,
			Model model) {
		String email = (String) session.getAttribute("email");
		// 관리자만 수정할수 있게끔, url요청으로 임의적으로 수정되지 않기 위해
		if (email.equals("dywlr74")) {
	...
	public String changeProposalOrderStatus(HttpSession session, OrderStatusRecord statusRecord,
	...
	public String adminOrderInfoAndRefundPro2(RefundComplete refund, HttpSession session, HttpServletRequest request,
			Model model) {
		String email = (String) session.getAttribute("email");
		// 관리자만 수정할수 있게끔, url요청으로 임의적으로 수정되지 않기 위해
		if (email.equals("dywlr74")) {
