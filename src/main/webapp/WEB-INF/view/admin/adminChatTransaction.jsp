...
<div class="title-div">구매/판매 거래내역(거래제안서)</div>
...
<table class="tran-table">
	...
	<tr>
		...
		<td></td>
		<td>
			<span class="myproposal">제안서 보기<span class="proposal-id">${pur.id}</span></span>
		</td>
	</tr>
	<tr>
	...
	<tr>
		...
		<td><c:if test="${pur.orderstatus=='입금대기'}"><span class="changeProposalOrderStatus">입금확인</span></c:if></td>
	</tr>
...
$(document).ready(function(){
	...
	$(document).on('click','.myproposal',function(){
		var id = $(this).find('.proposal-id').text();
		window.open("./myproposal?id="+id, "","width=700, height=600" );
	    });
	 
	
	
	 $(document).on('click','.changeProposalOrderStatus',function(){
 	  	var con = confirm("입금내역이 확인되셨습니까? \n 확인을 누르시면 결제완료 상태로 변경됩니다.");
 	  	if(con==true){
 	  		var chatTranId = $(this).closest('.tran-table').find(".proposal-id").text();
 	  		...
 	  		var orderData = {"depositor": depositorA, "buyerName": buyerNameA, 
 	  		 "bank": bankA, "chatTranId": chatTranId, "price": priceA};
 	  		
 	  		$.ajax({
 	  		  url:'./changeProposalOrderStatus',
 	  		  type:'post',
 	  		  data: orderData,
