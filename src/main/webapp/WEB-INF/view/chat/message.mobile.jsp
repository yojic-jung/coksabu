...
<body>
	...
	<div style="width: 100%; background: white; margin: auto; padding: 20px;">
		<c:if test="${size==0 }">
			<div style="text-align: left; font-size: 25px; font-family: 'JejuGothic'; color: gray;">대화상대가 없습니다.</div>
		</c:if>
		...
		<c:forEach var="pro" items="${pro}">
			<a class="chatMember-${pro.id}" href="./chatmyroom?id=${pro.id}" style="text-decoration: none; color: black;">
				<table style="border-spacing: 10px; font-family: JejuGothic; font-size: 35px; width: 100%; border-bottom: 1px solid gray;">
					<tr>
						<td rowspan="3" style="width: 202px;">
							<img style="width: 200px; height: 200px; border-radius: 100px;" src="<c:url value="/img/profileImg/${pro.imgPath}" />" alt="본인 프로필 이미지" />
						</td>
					...
					<tr>
						<td colspan="2" style="color: rgb(151, 149, 149); padding: 0px 5px 0px 20px; text-align: right;">${pro.lasttime}</td>
					</tr>
				</table>
			</a>
		</c:forEach>
