...
<body>
<div style="background:orange;font-size:30px; color:white;padding:20px;margin:15px; text-align:center; font-family:'JejuGothic';">선생님 레슨카드 지원 목록(${size}/10)</div>
<c:if test="${size==0}">
			...
			<% i++; %>
							<tr>
								<td colspan="2" style="text-align:center;">
									...
									<a href="./chatroom?id=${pro.teacher_id}&root=apply" style="color:gray;">채팅하기</a>
								</td>
							</tr>
	                            <tr>
                            ...
                            <tr>
                                ...
                                ${pro.universe} ${pro.univsub} <br/>
                                                        경력 <c:if test="${pro.career!=0}">${pro.career}년이상</c:if><c:if test="${pro.career==0}">1년미만</c:if>
                                </td>
                            </tr>
                            <tr>
                                <td class="locale" style="padding:0px 5px 0px 20px;word-break:keep-all;line-height:45px;">${pro.locales}</td>
                            </tr>
                            <tr>
...
<div style="height:300px;"></div>
	...
	$(document).ready(function(){
		...
		$('.m-jbMenu').append("선생님 지원 현황");
		
		 $('.boardread').click(function(){
			var id =  $(this).find('.postId').text();
			 window.location="./boardread?postId="+id;
		 });
